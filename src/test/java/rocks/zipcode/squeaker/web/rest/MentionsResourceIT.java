...
class MentionsResourceIT {
    ...
    void createMentions() throws Exception {
        ...
        // Create the Mentions
        MentionsDTO mentionsDTO = mentionsMapper.toDto(mentions);
        restMentionsMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(mentionsDTO)))
    ...
    void putNewMentions() throws Exception {
        ...
        MentionsDTO mentionsDTO = mentionsMapper.toDto(updatedMentions);

        restMentionsMockMvc
            .perform(
                put(ENTITY_API_URL_ID, mentionsDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(mentionsDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamMentions() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restMentionsMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(mentionsDTO)))
    ...
    void patchNonExistingMentions() throws Exception {
        ...
        restMentionsMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, mentionsDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(mentionsDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamMentions() throws Exception {
        ...
        restMentionsMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(mentionsDTO))
            )
            .andExpect(status().isMethodNotAllowed());
